"""
COPY OF ML_PLANNER() BUT WITH RENAMED CLASS NAME.
THIS COPY PREVENTS THE INIZIALIZATION WITH A CHECKPOINT PER DEFAULT.
SINCE THE SCENE MOTION MODEL ARCHITECHTURE IS NOT DEFINED WITHIN THIS REPOSITORY, THE CLASS CANNOT BE INIZIALIZED USING A CHECKPOINT.
INSTEAD THE MODEL CHECKPOINT IS LOADED WITHIN ITS MODEL CLASS IN:
    tuplan_garage.planning.training.modeling.models.scene_motion_model.SceneMotionModel
"""
...
class SceneMotionPlanner(AbstractPlanner):
    """
    ...
    def _infer_model(self, features: FeaturesType) -> npt.NDArray[np.float32]:
        ...
        trajectory = (
            trajectory_tensor.cpu().detach().numpy()[0]
        )  # retrive first (and only) batch as a numpy array
    ...
    def compute_planner_trajectory(
        ...
        features = self._model_loader.build_features(
